...
static void m_print(m_mpool_t *mpool, char *msg)
{
	...
	for (i = 0; i < N; ++i) {
		...
		kprintf("\n");
		vel = (i == 0 ? INKREMENT: vel * 2);
	}
	printf("<<<< %s\n", msg);
}

void *m_init_list( void *mem_segm, size_t size)
{
	size_t start, end;
...
void *m_alloc ( m_mpool_t *mpool, size_t size )
{
	...
	m_print( m_pool, "poslije dodjele");
...
int m_free ( m_mpool_t *mpool, void *chunk_to_be_freed )
{
	...
	/* set chunk tail */
	CLONE_SIZE_TO_TAIL ( chunk );
	m_print( m_pool, "nakon oslobadjanja" );
	return 0;
}
